...
(function (global) {
  ...
  var webGL = {
    ...
    Shader: function (params) {
      this.params = params;
      this.fragmentShader = webGL.getShaderByName(
      ...
      if (!webGL.gl.getProgramParameter(this.program, webGL.gl.LINK_STATUS)) {
        alert(
          "Unable to initialize the shader program: " +
            webGL.gl.getProgramInfoLog(this.program)
        );
      }
      ...
      for (var i = 0; i < this.params.attributes.length; i++) {
        ...
        this.attributes[attributeName] = webGL.gl.getAttribLocation(
          this.program,
          attributeName
        );
      ...
      for (i = 0; i < this.params.uniforms.length; i++) {
        ...
        this.uniforms[uniformName] = webGL.gl.getUniformLocation(
          this.program,
          uniformName
        );
      ...
      if (!webGL.gl.getShaderParameter(result, webGL.gl.COMPILE_STATUS)) {
        alert(
          "An error occurred compiling the shaders: " +
            webGL.gl.getShaderInfoLog(result)
        );
        return null;
      }
    ...
    drawScene: function (frameTime) {
      ...
      if (timing.prevFrameTime) {
        ...
        var update = quat.fromValues(
          controls.manualRotateRate[0] * interval,
          controls.manualRotateRate[1] * interval,
          controls.manualRotateRate[2] * interval,
          1.0
        );
      ...
      if (typeof vrHMD !== "undefined") {
        ...
        perspectiveMatrix = util.mat4PerspectiveFromVRFieldOfView(
          leftParams.recommendedFieldOfView,
          0.1,
          10
        );
        webGL.drawOneEye(0, perspectiveMatrix);
        perspectiveMatrix = util.mat4PerspectiveFromVRFieldOfView(
          rightParams.recommendedFieldOfView,
          0.1,
          10
        );
        ...
        if (timing.end - timing.frameTime > 20) {
          console.log(
            timing.framesSinceIssue +
              " Frame time: " +
              (timing.start - timing.frameTime) +
              "ms animation frame lag + " +
              (timing.canvasResized - timing.start) +
              "ms canvas resized + " +
              (timing.textureLoaded - timing.canvasResized) +
              "ms to load texture + " +
              (timing.end - timing.textureLoaded) +
              "ms = " +
              (timing.end - timing.frameTime) +
              "ms"
          );
